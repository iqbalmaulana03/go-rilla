...
func (p pgConfig) ConnStr() string {
	return fmt.Sprintf("host=%s port=%d database=%s user=%s password=%s sslmode=%s", p.Host, p.Port, p.DBName, p.DBUser, p.DBPassword, p.SslMode)
}
...
func (p *pgConfig) loadFromEnv() {
	...
	loadEnvStr("KAD_DB_USER", &p.DBUser)
	loadEnvStr("KAD_DB_PASSWORD", &p.DBPassword)
